...
def parseKernsetGPOS(name, lookup):
    ...
    for j in lookup[4:]:
        ...
        if "right x placement" in j_list[0]:
            try:
                int(j_list[1])
                one = "@_firstclass_group_" + str(j_list[1]) + "_" + name[2:-2]
            except:
            ...
            try:
                ...
                two = j_list[2]
            kernset_txt += "\tposition " + " ".join([one, two]) + " < " + j_list[3] + " 0 " + j_list[3] + " 0 >;\n"
    #     elif "y advance" in j_list[0]:
...
def mti2fea(family, insertInUfo=False, exportFea=True):
    # VARIABLES
    repo = "../src/" + family + "/"
